...
#
######################################################################
# $Id: tracert.cgi,v 1.1 2009/01/26 08:59:56 etitl Exp $
######################################################################
<? # 
...
cat <<-EOF
<p>
This page  allows you to run a traceroute command on your firewall 
</p>
EOF
######################################################################
...
EOF
	...
	Run )	# generate the network configuration file 
			...
			echo "<pre>"
			$TRACEROUTE $FORM_addr 2> /dev/null
			echo "</pre>"
			;; 
...
[ -r ./tracert.blurb ] && cat ./tracert.blurb
