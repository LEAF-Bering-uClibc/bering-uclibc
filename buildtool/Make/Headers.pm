package buildtool::Make::Headers;
...
sub make () {
    ...
    print "make the list of required headers packages: ";
...
sub _buildFiles($) {
    ...
    foreach $part (@dllist) {
        ...
        print "\nheaders for source/package: $part\n";
        print "------------------------\n";
        ...
        # now call make build
        $self->_callMake( "headers", $part, $localEnv );

        # add to list
        $self->addEntry( "headers", $part );
